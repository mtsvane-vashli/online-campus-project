...
import { createGLTFLoader } from '../utils/AssetLoader.js';
...
export async function createCampusEnvironment(scene, world, mode = 'day') {
    ...
    // 影のカメラの範囲を調整
    directionalLight.shadow.camera.left = -50; // 左端
    directionalLight.shadow.camera.right = 50; // 右端
    directionalLight.shadow.camera.top = 50; // 上端
    directionalLight.shadow.camera.bottom = -50; // 下端
    ...
    try {
        ...
        const gltf = await gltfLoader.loadAsync('/Kyushu_University_beta7.glb');
